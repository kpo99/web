module.exports = {
  ...
  _getBase: function(rowSpecs, colSpecs, opts) {
    ...
    if (opts.borders) {
      ...
      if(opts.borders.size)
        t_obj["@w:sz"] = opts.borders.size
      else
        t_obj["@w:sz"] = "12"

      if(opts.borders.color)
        t_obj["@w:color"] = opts.borders.color
      else
        t_obj["@w:color"] = "000000"

      if (opts.borders.inside) {
        if(opts.borders.inside.h) {
          t_obj_in_h = t_obj;
          if (opts.borders.inside.size)
            t_obj_in_h["@w:sz"] = opts.borders.inside.size
          else
            t_obj_in_h["@w:sz"] = "12"

          if (opts.borders.inside.color)
            t_obj_in_h["@w:color"] = opts.borders.inside.color
          else
            t_obj_in_h["@w:color"] = "000000"
        }
        if(opts.borders.inside.v){
          t_obj_in_v = t_obj;
          if (opts.borders.inside.size)
            t_obj_in_v["@w:sz"] = opts.borders.inside.size
          else
            t_obj_in_v["@w:sz"] = "12"

          if (opts.borders.inside.color)
            t_obj_in_v["@w:color"] = opts.borders.inside.color
          else
            t_obj_in_v["@w:color"] = "000000"
        }
      }
      ...
      baseTable["w:tbl"]["w:tblPr"]["w:tblBorders"] = {
        ...
        "w:left": t_obj,
        "w:right": t_obj,
        "w:insideH": t_obj_in_h,
        "w:insideV": t_obj_in_v
      };
    }
    return baseTable;
  },
